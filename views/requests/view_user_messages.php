				...
				<li 
					onclick="
						requests.load_messages({
							from_user: 1,
							from: '<?php echo $value['correo'] ?>',
							to: '<?php echo $value['estadomx'].'---'.$value['municipiomx'] ?>',
						...
						<div class="input-group input-group-lg">
							<input 
								onkeypress="if(event.keyCode == 13){requests.send_message({
									from_user: 1,
									from: '<?php echo $_SESSION['user']['correo'] ?>',
									to: requests.data_messages.estadomx+'---'+requests.data_messages.municipiomx,
